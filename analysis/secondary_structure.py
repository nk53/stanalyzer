...
DSSP_path = os.path.expanduser("~") + '/DSSP/dssp/bin/mkdssp'
...
def write_salt_bridge(psf: sta.FileRef, traj: sta.FileRefList, out: sta.FileRef,
                      sel: str = 'protein', interval: int = 1,) -> None:
    """Writes secondary structure to `out` file"""
    
    residue_list = []
    ss_all_frames = []
    step_num = 0
    random_tmp_filename = 'tmp'+''.join(random.choices(string.ascii_letters+string.digits, k=7))
    ...
    for traj_file in traj:
        ...
        for i in range(len(atom_names)):
            if i < len(atom_names) - 1 and atom_names[i] == 'OT1' and atom_names[i+1] == 'OT2': #Charmm FF
                atom_names[i] = 'O'
                atom_names[i+1] = 'OXT'
        ...
        if use_segid:
            # If the input system does not has chainIDs, add chainID (A to Z then a to z) to each segment
            # Could have issue if the system has more than 26 segments
            chainIDs = []
            chainID_to_segid = {}
            for i in range(all_atoms.n_segments):
                chainID = chr(65+i)
                all_atoms.segments[i].atoms.chainIDs = [chainID]*len(all_atoms.segments[i].atoms)
        ...
        for ts in u.trajectory:
            ...
            os.system('{} --output-format=dssp {}_{}.pdb {}_{}.dssp'.format(DSSP_path, random_tmp_filename, step_num,
                                                                            random_tmp_filename, step_num))
            processing_line = False
            ss_this_frame = []
            residue_list_this_frame = []
            for line in open('{}_{}.dssp'.format(random_tmp_filename, step_num)).readlines():
                ...
                if len(residue_list) == 0:
                    if use_segid:
                        residue_list_this_frame.append('{}_{}_{}'.format(chainID_to_segid[chainID], aa1to3.get(aa, 'XXX'), resid))
                    else:
                        residue_list_this_frame.append('{}_{}_{}'.format(chainID, aa, resid))
