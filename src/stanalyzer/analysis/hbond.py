...
def write_hbond(psf: sta.FileRef, traj: sta.FileRefList, out: sta.FileRef,
                sel: str = 'all', hydrogens_sel: str | None = None,
                acceptors_sel: str | None = None, d_a_cutoff: float = 3.0,
                d_h_a_angle_cutoff: float = 150.0, interval: int = 1,) -> None:
    ...
    if hydrogens_sel is None or hydrogens_sel.lower() == 'none':
        ...
        except Exception:  # TODO: narrow this clause
    ...
    with sta.resolve_file(out, 'w') as outfile:
        ...
        for entry in hbonds.results['hbonds']:
            ...
            e1seg = u.atoms[int(entry[1])].segid
            e3seg = u.atoms[int(entry[3])].segid
            skip_types = 'SOLV', 'IONS'
            if e1seg in skip_types and e3seg in skip_types:
                continue
            output = "{:.0f}".format(entry[0])
            for i in range(1, 4):
                atom = u.atoms[int(entry[i])]
                output += f" {atom.segid}_{atom.resname}_{atom.resid}_{atom.name}"
...
def get_parser() -> argparse.ArgumentParser:
    ...
    # parser.add_argument(
    #     '--donors-sel', metavar='selection', default=None,
