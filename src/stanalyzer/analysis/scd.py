...
def run_scd(sel, split, qz, qb, qt, qa, sel_sys, psf: sta.FileRef, traj: sta.FileRefList, interval: int = 1, center: bool = False) -> None:
    ...
    ag_ref = myscd.generate_ref_atomgroups(
        u, ag_full, ntype, nmol_type, ref_name)
    ...
    scd, weight = myscd.setup_scd_weight(
        nside, ntype, nchain, ncarbons, framenum, qb)
    ascd, astd = myscd.setup_ascd_astd(
        nside, ntype, nchain, ncarbons, qb)  # average & std
    ...
    for i in range(0, framenum):
        ...
        scd, weight = myscd.calculate_scd(
            i, ntype, raw_scd, scd, weight, id_side, id_type, nchain, ncarbons, qb)

    # Calculate weighted ave and std of SCD
    ascd, astd = myscd.calculate_ave_and_std(
        ascd, astd, scd, weight, framenum, qb)

    # Write output
    if qb:
        if qa:
            print('# Write average SCD')
            write_ave_std_bilayer(ntype, name_type, nchain,
                                  carbons, ncarbons, ascd, astd, odir1)

        if qt:
            print('# Write time series output')
            write_time_series_bilayer(
                framenum, interval, ntype, name_type, nchain, carbons, ncarbons, scd, odir2)

    else:  # if (qb is False):
        if qa:
            print('# Write average SCD')
            write_ave_std_leaflet(
                nside, ntype, name_type, sside, nchain, carbons, ncarbons, ascd, astd, odir1)
