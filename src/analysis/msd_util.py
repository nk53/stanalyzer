...
def init_unwrap_mol_com(pos,mass_mol,tmass_mol,pos_prev,pos_unwrap,com_unwrap,traj_com_unwrap):
  """
  ----------
...
def init_unwrap_sys_com(pos_sys,mass_sys,tmass_sys,pos_sys_prev,com_sys_unwrap,traj_com_sys_unwrap):
  """
  ----------
...
def update_unwrapped_mol_com_traj(iframe,pos_unwrap,mass_mol,tmass_mol,com_unwrap,traj_com_unwrap):
  """
  ----------
...
def calculate_msd(taus,framenum,traj_com_unwrap,id_type,msd):
  ...
  for i in range(0,ntau):
    tau = taus[i]
    if (tau > framenum - 1): break
    if (tau % 100 == 0): print(f'MSD progress {tau}/{taus[-1]}')
  
    # Calculate MSD(tau) 
    tmsd = calculate_msd_tau(tau,framenum,ntype,id_type,traj_com_unwrap)
...
def calculate_msd_bilayer(msd,nside,ntype,ntaus,nmol_type):
  ...
  bnmol_type = np.sum(nmol_type,axis=0) # sum over leaflets - nomralization factor
  ...
  for i in range(0,nside):
    for j in range(0,ntype):
      for k in range(0,ntaus):
        for m in range(0,3):
          bmsd[j,k,m] += msd[i][j,k,m] * nmol_type[i][j] / bnmol_type[j]
